...
class ReverseFootComponent(keyablecomponent.KeyableComponent):
    LATEST_VERSION = 1
    
    @staticmethod
    def create(metanode_parent, start_joint, end_joint, side, region, heel_chain_name='heel_contact', ball_chain_name='ball_contact', ball_lift_axis='x', toe_lift_axis='x', toe_pivot_axis='y', heel_lift_axis='x', foot_lean_axis='z', ball_pivot_axis='y'):
        ...
        reversefootnode = keyablecomponent.KeyableComponent.create(metanode_parent, ReverseFootComponent.__name__, ReverseFootComponent.LATEST_VERSION, side, region, component_pivot = hip_joint)
        chain_between = joint_chain + heel_contact_joint + ball_contact_joint
        #metanode_root = metaroot.get_metaroot(metanode_parent)
        
        do_not_touch = reversefootnode.get_do_not_touch()
        ctrls_group = reversefootnode.get_ctrls_group()
        ...
        ik_foot_toe_contact = ik_leg_chain[4] #toe_contact_null_dupe
        ...
        #if 4 joint chain last joint rotation moves with ik flag
        ...
        toe_ik, toe_eff = pm.ikHandle(sj= ik_foot, ee=ik_foot_toe, n='{0}_{1}_toe_ikhandle'.format(side_prefix, region))
        ...
        invert_node.straight >> ik_toe_grip.v #otherwise, this grip is included in the check for visible grips
        invert_node.straight >> pv_grip.v

        #xnode connections required for freeform
        reversefootnode.connect_ordered_nodes_to_metanode(joint_chain[:-1], 'bind_joints')
        ...
        foot_ik_grip.addAttr('toe_tap', k=True, at='double')
        foot_ik_grip.addAttr('ball_pivot', k=True, at='double')
        ...
        foot_ik_grip.ball_pivot >> reverse_toe.attr('rotate'+ ball_pivot_axis.upper())
        foot_ik_grip.ball_pivot >> reverse_ball_contact.attr('rotate'+ ball_pivot_axis.upper())
        toe_orient_constraint = pm.listConnections(toe_joint, type='orientConstraint')[0]
        toe_tap_add = pm.createNode("plusMinusAverage", n = name_root + "toe_tap_add")
        foot_ik_grip.toe_tap >> toe_tap_add.input3D[0].input3Dx
        ik_toe_grip.rotate >> toe_tap_add.input3D[1]
        toe_tap_add.output3D >> reverse_toe_wiggle.rotate
        
        #xnode connections
        reversefootnode.connect_node_to_metanode(ik, 'ik_leg_handle')
        reversefootnode.connect_node_to_metanode(pv_grip, 'pv_grip')
        reversefootnode.connect_node_to_metanode(pv_line, 'pv_line')
        reversefootnode.connect_node_to_metanode(switch_grip, 'switch_grip')
        reversefootnode.addAttr("switch_attr_name", dt= "string")
        reversefootnode.attr("switch_attr_name").set(switch_attr_name)
        reversefootnode.connect_node_to_metanode(foot_ik_grip, 'ik_grip')
        reversefootnode.connect_node_to_metanode(ik_toe_grip, 'ik_toe_grip')
        #reversefootnode.connect_node_to_metanode(ik_toe_pair_blend, 'ikToePairBlend')
        reversefootnode.connect_node_to_metanode(reverse_heel_contact, 'rev_heel_contact')
        reversefootnode.connect_node_to_metanode(reverse_toe_contact, 'rev_toe_contact')
        reversefootnode.connect_node_to_metanode(reverse_toe_wiggle, 'rev_toe_wiggle')
        reversefootnode.connect_node_to_metanode(reverse_toe, 'rev_toe')
        reversefootnode.connect_ordered_nodes_to_metanode(fk_grips, 'fk_grips')
        reversefootnode.connect_node_to_metanode(fk_foot_toe, 'fk_toe_grip')
        reversefootnode.connect_ordered_nodes_to_metanode(fk_leg_chain, 'fk_chain')
        reversefootnode.connect_ordered_nodes_to_metanode(ik_leg_chain, 'ik_chain')
        ...
        miscutil.lock_all_keyable_attrs(reverse_heel_contact)
        miscutil.lock_all_keyable_attrs(reverse_toe_contact)
        miscutil.lock_all_keyable_attrs(reverse_toe_wiggle)
        miscutil.lock_all_keyable_attrs(reverse_toe)
        
        miscutil.lock_and_hide_attrs(foot_ik_grip, ['sx', 'sy', 'sz', 'v'])
        miscutil.lock_and_hide_attrs(ik_toe_grip, ['tx', 'ty', 'tz', 'sx', 'sy', 'sz', 'v'])
    ...
    def attach_to_skeleton(self, namespace = None):
        ...
        constraints = []
        constraints.append(miscutil.safe_parent_constraint(ankle_joint, ik_grip, w = 1))
        constraints.append(miscutil.safe_parent_constraint(mid_joint, pv_locator, w = 1, mo = 1))
        constraints.append(pm.parentConstraint(pv_locator, pv_grip, w = 1, skipRotate = ["x", "y", "z"]))
        constraints.append(pm.parentConstraint(toe_joint, toe_grip, w = 1, skipTranslate = ["x", "y", "z"]))
    ...
    def get_ik_toe_grip(self):
        return rigging.Grip(self.ik_toe_grip.listConnections()[0])
    ...
    def get_grips(self):
        grips = [self.ik_toe_grip.listConnections()[0], self.pv_grip.listConnections()[0], self.switch_grip.listConnections()[0], self.ik_grip.listConnections()[0] ]
